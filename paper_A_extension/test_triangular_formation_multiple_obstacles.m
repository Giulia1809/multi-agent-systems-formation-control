%In this implementation, obstacle avoidance is implemented on an elliptical trajectory, analogous to the one in the paper: "Linear Model Predictive Control via Feedback Linearization for Formation Control of Multiple Wheeled Mobile Robots", M. Kamel and Y. Zhang.
%Thus, the considered trajectory and leader-followers formation are more complex than the ones in the original paper. 
%In this case, several obstacles are in the path of the robots.
...
%obstacles list
obstacle_pos = {[0.0, -0.4], [0.0, -0.8], [0.2, 0.5], [0.2, 0.8], [0.2, 1.5], [0.2, 1.9] }; %cell array
%obstacle_pos = {[0.2, 0.5], [0.2, 1.5]};   %from v1
%obstacle_pos = {[0.1, 1.0], [0.1, 0.75]};  %from v1
...
for elapse_time = step:step:T
    ...
    %[c_ul] = obstacleCorrection(leader.q, obstacle_pos);
    [c_ul] = obstacleCorrection_v2(leader.q, obstacle_pos);
    ...
    %get obstacles correction
    %[c_u1] = obstacleCorrection(follower_1.q, obstacle_pos);
    %[c_u2] = obstacleCorrection(follower_2.q, obstacle_pos);
    [c_u1] = obstacleCorrection_v2(follower_1.q, obstacle_pos);
    [c_u2] = obstacleCorrection_v2(follower_2.q, obstacle_pos);
...
for ind = 1:size(obstacle_pos, 2)
        ...
        circles(obs(1), obs(2), .03, 'facecolor', 'yellow');
end
ylabel('y [m]');
xlabel('x [m]');
saveas(gcf, 'plots_elliptical_multiObstacle/trajectories_des.png');
...
ylabel('y [m]');
xlabel('x [m]');
saveas(gcf, 'plots_elliptical_multiObstacle/trajectories.png');
...
saveas(gca, 'plots_elliptical_multiObstacle/distances.png');
...
saveas(gca, 'plots_elliptical_multiObstacle/angles.png');
