...
obstacle_pos = {[5, 2, 0.2], [-1, 4.5, 0.2], [-2, 0, 0.3], [-4.5, -4, 0.5], [1, 3, 0.5] }; %cell array
%leader initial position
q_s = [3.8, -0.2, deg2rad(100)]';
leader = Unicycle(q_s);
...
leader_traj = trajectoryEllipticalv2(0.25, 0.25, 4);
...
Q_leader = 2 * eye(4,4);   %state cost matrix
Q_leader(3,3) = 0.01;
Q_leader(4,4) = 0.01;
R_leader = 5*eye(2,2);
...
Q_1 = 0.05 * eye(4,4);   %state cost matrix
Q_1(3,3) = 0.5;
Q_1(4,4) = 0.5;
R_1 = 5*eye(2,2);
...
Q_2 = 0.05 * eye(4,4);   %state cost matrix
Q_2(3,3) = 0.005;
Q_2(4,4) = 0.005;
R_2 = 2*eye(2,2);
...
for elapse_time = step:step:T
    ...
    [c_ul] = obstacleCorrectionVariableObs(leader.q, obstacle_pos, F_max_leader, s_leader, d_max_leader);
    ...
    %get obstacles correction
    [c_u1] = obstacleCorrectionVariableObs(follower_1.q, obstacle_pos, F_max_1, s_1, d_max_1);
    [c_u2] = obstacleCorrectionVariableObs(follower_2.q, obstacle_pos, F_max_2, s_2, d_max_2);
    ...
    if ~exist("tuning",'var')
        ...
        for ind = 1:size(obstacle_pos, 2)
            ...
            d = norm(obs(1:2)' - follower_2.q(1:2));
            if d <= obs(3) + 1 % to tune
                pos_err_2 = pos_err_2 + ( 1 / d);
                if d <= obs(3)
                    pos_err_2 = pos_err_2 + Inf;
                end
            end
        end

        total_error = total_error + pos_err_2;% + pos_err_1 + pos_err_2;
...
for ind = 1:size(obstacle_pos, 2)
        obs = obstacle_pos{ind};
        circles(obs(1), obs(2), obs(3), 'facecolor', 'black', 'HandleVisibility','off');
end
...
ylabel('y [m]');
xlabel('x [m]');
saveas(gcf, 'plot_elliptical_traj/trajectories_des.png');
...
circles(q_s(1), q_s(2), 0.03, 'facecolor', 'blue', 'HandleVisibility','off');
circles(q_s_f1(1), q_s_f1(2), 0.03, 'facecolor', 'black', 'HandleVisibility','off');
circles(q_s_f2(1), q_s_f2(2), 0.03, 'facecolor', 'red','HandleVisibility','off');
...
circles(leader.q(1), leader.q(2), 0.03, 'facecolor', 'blue', 'HandleVisibility','off');
circles(follower_1.q(1), follower_1.q(2), 0.03, 'facecolor', 'black', 'HandleVisibility','off');
circles(follower_2.q(1), follower_2.q(2), 0.03, 'facecolor', 'red', 'HandleVisibility','off');
ylabel('y [m]');
xlabel('x [m]');
saveas(gcf, 'plot_elliptical_traj/trajectories.png');
...
ylabel('w [rad/s]');
xlabel('t [s]');
saveas(gcf, 'plot_elliptical_traj/v_traj.png');
...
ylabel('errors in x,y [m]');
xlabel('t [s]');
saveas(gcf, 'plot_elliptical_traj/errors.png');
